...
<script type="text/javascript">
						...
						$('#dateRecpt, #timeOfPos, #timeOfActivation, #boatetd,#boateta, #vesseleta, #vesseletd')
									.datetimepicker({
							ignoreReadonly: true,
							widgetPositioning:{
						        horizontal: 'auto',
								...
								if($('#boatName').val()!=''){//below 2905
									...
									if(botConLen.length <10 && botConLen!=''){
										...
										$('#typeOfBoat option:selected').text()=='Select'?boatType='':boatType=$('#typeOfBoat option:selected').text();
										 ...
										 $('#typeOfBoat,#boatColrcd').val('').trigger('change');
...
Ok: function () {
	...
	function dataTableUpdateCrew(crArr,obj)
	{
		$('#addCrewButton').hide();
			...
			$('#crDtlsIdType').val(crwArray[10]).trigger("change");
			$('#crDtlsIdPf').val(crwArray[11]);
			...
			if(crewDataList.length==0){
				...
				$('#crew_det').modal('toggle'); 
						...
						function submitThreatDetails(){
							...
							//console.log("uploadedInputId"+uploadedInputId);
								  $.ajax({
						...
						$('#sharedWithName').change(function(){
							...
							console.log($('#sharedWithName').val() +"  >>>>");
...
<script>
						...
						$('#gpslattitudeDec').keyup(function(){
						 ...
						 $('#gpslongitudeDec').keyup(function(){
						            ...
						            var sec = frac * 3600 - min * 60;
						           // var show=deg + " degree " + min + " minutes " + sec.toFixed(2) + " seconds";
						           var show=deg + "\u00B0" + min + " ' " + sec.toFixed(2) + " '' " ;//(0306)
						            if(dd==''){
						            	 $('#gpslongitudeDeg').val('');
						            }
						            else{
