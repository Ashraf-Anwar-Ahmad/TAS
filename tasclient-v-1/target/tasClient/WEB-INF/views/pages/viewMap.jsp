...
<div class="form_map">
		...
		<label>Select Threat</label>
			 <select id="first_cod" class="form-control" >
...
mymap.on('click', function(e) {
	...
	L.marker(_firstLatLng).addTo(mymap).bindPopup('Point '+i+'<br/>' + e.latlng).openPopup();
 	
	
    var ftltlo=_firstLatLng.toString();
    ftltlo=ftltlo.replace(/[LatLng() ]/g,'');
    $('#sect_cod').val(ftltlo);
    firstCheck=ftltlo.split(",");
    
    
  });
  
$('#get_dist').click(function(){
	var scndlo=$('#first_cod option:selected').val();
    secondCheck=scndlo.split(",");
	if(firstCheck!='' && secondCheck!=''){
		...
		$.confirm({
		    title: 'Alert!',
		    content: 'Please select dropdown',
		    theme: 'material',
		    type: 'orange',
		...
		$.confirm({
		    title: 'Alert!',
		    content: 'Please select dropdown and click on map',
		    theme: 'material',
		    type: 'orange',
		    columnClass: 'col-md-3 offset-md-4',
		    buttons: {
...
var  contraband = new LeafIcon({iconUrl: 'static/img/mapicon/contra.png'}),
narcotic = new LeafIcon({iconUrl: 'static/img/mapicon/narcotics.png'}),
counterTerror = new LeafIcon({iconUrl: 'static/img/mapicon/counter_terorism.png'}),
humanTraffic = new LeafIcon({iconUrl: 'static/img/mapicon/humn.png'}),
satellite = new LeafIcon({iconUrl: 'static/img/mapicon/satellite.png'});
others = new LeafIcon({iconUrl: 'static/img/mapicon/other.png'});
...
//end dropdwon
for(i in test){
	//alert("check=="+Number(test[i].longitude));
	var threatTp=test[i].threatType;
	var p = new L.LatLng(test[i].longitude, test[i].lattitude);
	if(threatTp=='Contraband'){
		myIcon = contraband;
	}
	else if(threatTp=='Counter Terrorism'){
		myIcon = counterTerror;
	}
	else if(threatTp=='Narcotics'){
		myIcon = narcotic;
	}
	else if(threatTp=='Human Traffic'){
		myIcon = humanTraffic;
	}
	else if(threatTp=='Satellite'){
		myIcon = satellite;
	}
	else {
		myIcon = others;
	}
	var logtd=Number(test[i].longitude);
	var latdd=Number(test[i].lattitude);
	
	 L.marker([latdd, logtd], {icon: myIcon}).bindPopup("("+latdd+","+logtd+")").addTo(mymap);
	 
	 //below for dropdown
      html +="<option>"+latdd+","+logtd+"</option>"; 
