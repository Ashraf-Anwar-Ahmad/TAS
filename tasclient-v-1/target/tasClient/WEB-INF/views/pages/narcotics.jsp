										...
										<div class="multisteps-form__content">
											...
											<div class="row">
												...
												<div class="col-md-3 otherUnitInvolved"
													style="display: none;">
													<div class="form-group">
														<label>Other Unit Involved</label> <input type="text"
															class="form-control" name="otherUnit" id="otherUnit" />
													</div>
												</div>
												...
												<div class="col-md-3">
													<div class="form-group">
														<label>Joint operation with</label> <input type="text"
															class="form-control" name="jointOperWith"
															id="jointOperWith" />
													</div>
												</div>
...
$('#dateRecpt, #timeOfPos, #timeOfActivation, #boateta, #boatetd, #vesseleta, #vesseletd, #narcoDate').datetimepicker({
	ignoreReadonly: true,
	widgetPositioning:{
        horizontal: 'auto',
        vertical: 'bottom'
		...
		if(boolCheck)
  		{
			...
			if(ntynm!=''){
				...
				$('#narcoUnit option:selected').text()=='Select'?narcoUnit='':narcoUnit=$('#narcoUnit option:selected').text();
					 ...
					 $('#narco_table').dataTable().fnAddData( [
						 ...
						 ncqty,
						 narcoUnit,
							'<button type="button" onclick="dataTableUpdateNarco(\''+ntynm+'\',\''+ncqty+'\',\''+ncUnit+'\',this)" class="btn btn-blue btn-sm1"><i class="fa fa-edit"></i></button> <button type="button" onclick="dataTableDeleteNarco(\''+ntynm+'\',\''+ncqty+'\',this)" class="btn btn-danger btn-sm1"><i class="fa fa-remove"></i></button>'] );
					 var dataNarco = {};
					 dataNarco["narcoTypeName"] = ntynm;
...
function dataTableUpdateNarco(value,text,unit,obj)
{
	$('#addNarcoButton').hide();
	$('#updateNarcoButton').show();
	narUpd=1;//(0206)
	$('#narcoEye').attr('disabled',true);
	narcoDtlsList = $.grep(narcoDtlsList, function(el, idx) {return el.narcoTypeName == value}, true)
	//alert("data=="+JSON.stringify(narcoDtlsList));
	$('#narcoTypeName').val(value).trigger('change');
	$('#narcoQty').val(text);
	$('#narcoUnit').val(unit).trigger('change');
	$('#narco_det').modal('toggle');
	$('#narco_table').DataTable().row($(obj).parents('tr')).remove().draw(false);
}

function dataTableDeleteNarco(value,text,obj)
{
		narcoDtlsList = $.grep(narcoDtlsList, function(el, idx) {return el.narcoTypeName == value}, true)
		//alert("data=="+JSON.stringify(narcoDtlsList));
		$('#narco_table').DataTable().row($(obj).parents('tr')).remove().draw(false);
		$("#sizeNarco").text(narcoDtlsList.length+' records added');
		if(narcoDtlsList.length==0){
			$(".btNarco").hide();
			$('#narco_det').modal('toggle'); 
		}
} 
...
function submitThreatDetails(){
	...
	data["narcoDate"]=$('#narcoDate').val();
	data["broughtTo"]=$('#broughtTo').val();
 ...
 $.ajax({
		url : 'saveNarcoticDetails?uploadedInputId='+uploadedInputId,
