							...
							<div class="col-12   ml-auto mr-auto mb-4">
								<div class="multisteps-form__progress">
									...
									<button class="multisteps-form__progress-btn" type="button"
										title="Contraband Smuggling  Details" disabled="disabled">Narcotics
										Details</button>
									<button class="multisteps-form__progress-btn" type="button"
							...
							<div class="col-12 m-auto">
								<form class="multisteps-form__form">
									...
									<div class="multisteps-form__panel shadow p-4 rounded bg-white"
										data-animation="scaleIn">
										<h3 class="multisteps-form__title">Narcotics Smuggling
											Details</h3>
										<div class="multisteps-form__content">

											<!--Narcotics Smuggling details form start-->

											<div class="row">

												<div class="col-md-3">
													<div class="form-group">
														<label>Date and Time of Apprehension<span
															class="mandatory-field">*</span></label> <input type='text'
															class="form-control" id='narcoDate' name="narcoDate"
															readonly style="background-color: #ffff" />
													</div>
												</div>
												...
												<div class="col-md-3">
													<div class="form-group">
														<label>Brought to location</label> <input type="text"
															class="form-control" name="broughtTo" id="broughtTo" />
													</div>
												</div>
											...
											<div class="row">
												...
												<div class="col-12 col-md-6">
													<div class="inner_legd">
														<fieldset class="scheduler-border">
															<legend class="scheduler-border">Narcotics Type
																Details</legend>
															<div class="row">
																<div class="col-md-6">
																	<div class="form-group">
																		<label>Type of Narcotics</label> <select
																			class="form-control select2" name="narcoTypeName"
																			id="narcoTypeName">
																			<option value="">Select</option>
																			<c:forEach items="${narcoList}" var="narcoListItem">
																				<option value="${narcoListItem.id}">${narcoListItem.name}</option>
																			</c:forEach>
																		</select>
																	</div>
																	<div class="form-group">
																		<label>Units</label> <select
																			class="form-control select2" name="narcoUnit"
																			id="narcoUnit">
																			<option value="">Select</option>
																			<c:forEach items="${weightUnitList}"
																				var="weightListItem">
																...
																<div class="col-md-6">
																	<div class="form-group">
																		<label>Quantity(In Number)</label> <input
																			type="number" class="form-control"
																			onkeydown="return true" name="narcoQty" id="narcoQty"
																			min="0" step="any" />
																	</div>
																	<div class="form-group">
																		...
																		<div class="btNarco" style="display: none">
																			<button class="btn btn-default" id="narcoEye"
																				...
																				data-target="#narco_det">
																				<i class="fa fa-eye"></i> <span id="sizeNarco"></span>
																			</button>
																		</div>
																	</div>
																</div>
...
<div class="modal fade pdr0" id="narco_det" data-backdrop="static"
	tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
	aria-hidden="true">
	<div class="modal-dialog modal-md" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="exampleModalLabel">Narcotic Details</h5>
				<button type="button" class="close" data-dismiss="modal"
					aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			...
			<div class="modal-body">
				<div class="table-responsive-lg table-responsive-md">
					<table id="narco_table" class="table table-striped table-bordered">
						<thead class="thead-dark">
							<tr class="info tb-hd">
								<th>Type of Narcotics</th>
								<th>Quantity</th>
								<th>Unit</th>
								<th>Action</th>
...
$('#dateRecpt, #timeOfPos, #timeOfActivation, #boatetd, #boateta, #vesseleta, #vesseletd, #narcoDate').datetimepicker({
	ignoreReadonly: true,
	widgetPositioning:{
        horizontal: 'auto',
        vertical: 'bottom'
...
$(document).ready(function() {
	...
	narcoDtlsList = test.narcoticList;
console.log(JSON.stringify(test) + " >>test");
	...
	$('#contact_tab').dataTable().fnAddData( [
		...
		contactDtlsList[i].country,
			'<button type="button" onclick="dataTableUpdateContact(\''+contactDtlsList[i].contactNo+'\',\''+contactDtlsList[i].country+'\',\''+contactDtlsList[i].cntctId+'\',this)" class="btn btn-blue btn-sm1"><i class="fa fa-edit"></i></button>',
			'<button type="button" onclick="dataTableDeleteContact(\''+contactDtlsList[i].contactNo+'\',\''+contactDtlsList[i].country+'\',\''+contactDtlsList[i].cntctId+'\',this)" class="btn btn-danger btn-sm1"><i class="fa fa-remove"></i></button>'] );
	...
	for(i in narcoDtlsList){
		var arr_narco=[];
		arr_narco.push(narcoDtlsList[i].narcoTypeId);
		arr_narco.push(narcoDtlsList[i].narcoQty);
		arr_narco.push(narcoDtlsList[i].narcoUnit);
		
		console.log(""+ JSON.stringify(narcoDtlsList));
		$('#narco_table').dataTable().fnAddData( [
			narcoDtlsList[i].narcoTypeName,
			narcoDtlsList[i].narcoQty,
			narcoDtlsList[i].narcoUnitDescr,
			'<button type="button" onclick="dataTableUpdateNarco(\''+narcoDtlsList[i].narcoTypeId+'\',\''+narcoDtlsList[i].narcoTypeId+'\',\''+narcoDtlsList[i].narcoQty+'\',\''+narcoDtlsList[i].narcoUnit+'\',\''+narcoDtlsList[i].narcoId+'\',this)" class="btn btn-blue btn-sm1"><i class="fa fa-edit"></i></button> <button type="button" onclick="dataTableDeleteNarco(\''+narcoDtlsList[i].narcoTypeId+'\',\''+narcoDtlsList[i].narcoQty+'\',\''+narcoDtlsList[i].narcoUnit+'\',\''+narcoDtlsList[i].narcoId+'\',this)" class="btn btn-danger btn-sm1"><i class="fa fa-remove"></i></button>'] );
	...
	$("#sizeNarco").text(narcoDtlsList.length+' records added');
	
	console.log(narcoDtlsList.length+" >length");
	
	narcoDtlsList.length>0?$(".btNarco").show():$(".btNarco").hide();
...
//	$('#imeiNo').val(test.imeiNo);
	...
	$('#narcoDate').val(test.narcoDate);
	$('#broughtTo').val(test.broughtTo);
	$('#worth').val(test.worth);
	$('#caseHandedTo').val(test.caseHandedTo);
	$('#jointOperWith').val(test.jointOperWith);
...
//below for narcotics details
<script>
var narcoDelList=[];
var narcoDelChk=[];
var narcId;
function fnClickAddNarco() {
		//$('#giffy').show();
		var boolCheck=false;
		var filteredFamily = narcoDtlsList.filter(function(x) {
  		if (x.narcoTypeName==$("#narcoTypeName").val()) {
   			//alert("check1");
   			boolCheck=true;
  		}
		});
		if(boolCheck)
  		{
			$.confirm({
			    title: 'Alert!',
			    content: 'You have already added the same Narcotic type',
			    theme: 'material',
			    type: 'orange',
			    buttons: {
			        Ok: function () {
		...
		var ntynm=$('#narcoTypeName').val();
		var ncqty=$('#narcoQty').val();
		var ncunit=$('#narcoUnit').val();
		
		narcId==null || narcId=='' ? narcId='' : narcId=narcId+'-U';
		var chqq=ntynm;
		if(ntynm!=''){
			$('#addNarcoButton').show();
			$('#updateNarcoButton').hide();
			$('#narcoEye').attr('disabled',false);
			narUpd=0;//(0206)
			var ntytxt;
			var nUnitText;
			$('#narcoTypeName option:selected').text()=='Select'?ntytxt='':ntytxt=$('#narcoTypeName option:selected').text();
			
			$('#narcoUnit option:selected').text()=='Select'?nUnitText='':nUnitText=$('#narcoUnit option:selected').text();
			
			$('#narco_table').dataTable().fnAddData( [
					 ntytxt,
					 ncqty,
					 nUnitText,
						'<button type="button" onclick="dataTableUpdateNarco(\''+ntynm+'\',\''+chqq+'\',\''+ncqty+'\',\''+ncunit+'\',\''+narcId+'\',this)" class="btn btn-blue btn-sm1"><i class="fa fa-edit"></i></button> <button type="button" onclick="dataTableDeleteNarco(\''+ntynm+'\',\''+ncqty+'\',\''+ncunit+'\',\''+narcId+'\',this)" class="btn btn-danger btn-sm1"><i class="fa fa-remove"></i></button>'] );
				 var dataNarco = {};
				 dataNarco["narcoTypeId"] = ntynm;
				 dataNarco["narcoQty"] = ncqty;
				 dataNarco["narcoId"] = narcId;
				 dataNarco["narcoUnit"] =ncunit;
				 narcoDtlsList.push(dataNarco);
				 
				 $('#narcoQty').val('');
				 $('#narcoTypeName').val('').trigger('change');
				 $('#narcoUnit').val('').trigger('change');
				 narcId='';
				 
				 console.log("data=="+JSON.stringify(narcoDtlsList));
				 $("#sizeNarco").text((narcoDtlsList.length)-(narcoDelChk.length)+' records added');
				 $(".btNarco").show();
			...
			$.confirm({
			    title: 'Alert!',
			    content: 'Please add atleast narcotics type',
			    theme: 'material',
			    type: 'orange',
			    buttons: {
			        Ok: function () {
...
function dataTableUpdateNarco(value,chk,text,text2,obj)
{
	
	console.log("value>"+value+ "chk=="+chk+"text=="+text+"text2=="+text2);
	narUpd=1;//(0206)
	$('#addNarcoButton').hide();
	$('#updateNarcoButton').show();
		$('#narcoEye').attr('disabled',true);
		if(text2!='' && text2!=null){
			var chnCrId=text2.split('-');	
			narcId=chnCrId[0];
			}
		
		narcoDtlsList = $.grep(narcoDtlsList, function(el, idx) {return el.narcoTypeId == value}, true)
		console.log("data=="+JSON.stringify(narcoDtlsList));
		$('#narcoTypeName').val(chk).trigger('change');
		$('#narcoQty').val(text);
		$('#narcoUnit').val(text2).trigger('change');
		$('#narco_det').modal('toggle');
		$('#narco_table').DataTable().row($(obj).parents('tr')).remove().draw(false);
}

function dataTableDeleteNarco(value,text,text2,obj)
{
	//alert("check=="+text2);
	if(text2!='' && text2!=null){
		var dataNarcDel = {};
		var delrcd=text2.split('-');
		dataNarcDel["narcoId"] = delrcd[0]+'-D';
		narcoDelList.push(dataNarcDel);
		narcoDelChk.push(dataNarcDel);
	}
		narcoDtlsList = $.grep(narcoDtlsList, function(el, idx) {return el.narcoTypeId == value}, true)
		//alert("data=="+JSON.stringify(narcoDtlsList));
		$('#narco_table').DataTable().row($(obj).parents('tr')).remove().draw(false);
		Array.prototype.push.apply(narcoDtlsList,narcoDelList);
		narcoDelList=[];
		$("#sizeNarco").text((narcoDtlsList.length)-(narcoDelChk.length)+' records added');
		if((narcoDtlsList.length)-(narcoDelChk.length)==0){
			$(".btNarco").hide();
			$('#narco_det').modal('toggle'); 
		}
		//alert("data merged=="+JSON.stringify(narcoDtlsList));
} 
		...
		if($('#boatName').val()!=''){//below 2905
			...
			if(botConLen.length<10 && botConLen!=''){
				...
				$('#typeOfBoat option:selected').text()=='Select'?boatType='':boatType=$('#typeOfBoat option:selected').text();
				 ...
				 dataBoat["typeOfBoat"] = arr_boat[0];
				 dataBoat["boatName"]=  arr_boat[1];
				 dataBoat["regNum"] = arr_boat[2];
				 dataBoat["ownerName"] = arr_boat[3];
				 dataBoat["stateReg"] = arr_boat[4];
				 dataBoat["portReg"] = arr_boat[5];
				 dataBoat["noOfCrew"] = arr_boat[6];
				 dataBoat["docHeld"] = arr_boat[7];
				 dataBoat["navEqpt"] = arr_boat[8];
				 dataBoat["commEqpt"] = arr_boat[9];
				 dataBoat["lifeEqpt"] = arr_boat[10];
				 dataBoat["boatlpc"] = arr_boat[11];
				 dataBoat["boatetd"] = arr_boat[12];
				 dataBoat["boatnpc"] = arr_boat[13];
				 dataBoat["boateta"] = arr_boat[14];
				 dataBoat["licenseFish"] = arr_boat[15];
				 dataBoat["ownerContCode"] = arr_boat[16];
				 dataBoat["ownerContDts"] = arr_boat[17];
				 dataBoat["boatColrcd"] = arr_boat[18];
				 dataBoat["remarks"] = arr_boat[19];
...
$(document).on("click",".removeFile", function(e){
   ...
   // to takes uploaded file names
    ...
    $("#uploadBtn").click(function (e) {
        ...
        var threatFolder = "NarcoticsSmuggling" ;
               			...
               			if(response.Status == 200){
               		    	...
               		    	// to remove all selected file  
    	           			 ...
    	           			 for (i = 0; i < obj.length; i++) { 
    	           			     ts+= "<li><a   href=\"viewdoc?fileName="+obj[i]+"&inputId=${inputId}&threatFolder=NarcoticsSmuggling\" target=\"_blank\"> <strong class=\"t\">"+obj[i]+"</strong>  </a>"
...
function submitThreatDetails(){
	...
	data["broughtTo"]=$('#broughtTo').val();//(1406)
	data["narcoDate"]=$('#narcoDate').val();//(1406)
	...
	data["narcoticList"]=narcoDtlsList;
...
function gncheck2(){
	...
	if(narUpd==1){//(0206) start
		$('#msgg').show();
		$('#addMessage').text(' You have unsaved changes in Narcotic details, please click on update button');
		callCheck();
	}//end
	else{
		return true;
