							...
							<div class="col-12   ml-auto mr-auto mb-4">
								<div class="multisteps-form__progress">
									...
									<button class="multisteps-form__progress-btn" type="button"
										title="General Details" disabled="disabled">Satellite
										Interception Details</button>
									<button class="multisteps-form__progress-btn" type="button"
										title="Boat/Vessels Details" disabled="disabled">Boat/Vessels
										Details</button>
									<button class="multisteps-form__progress-btn" type="button"
										title="Other Information" disabled="disabled">Crew
										Details</button>
									<button class="multisteps-form__progress-btn" type="button"
...
$('#dateRecpt, #timeOfPos, #timeOfActivation, #boateta, #boatetd, #vesseleta, #vesseletd,#satActTime').datetimepicker({
	ignoreReadonly: true,
	widgetPositioning:{
        horizontal: 'auto',
        vertical: 'bottom'
...
$("#files1").fileUploader(filesToUpload);
 ...
 // upload function
    ...
    //alert("uploadTime==="+uploadTime);
     ...
     var threatFolder = "satelliteInterception" ;
...
function submitThreatDetails(){
 ...
 $.ajax({
		url : 'saveSatelliteInterception?uploadedInputId='+uploadedInputId,
...
$('input[name="boat_vessel_det"]').click(function(){
	...
	console.log("   <<<<<inputValue>>>>   " +inputValue+"   <<<<<vesselDataList>>>>   " +  vesselDataList.length +"   <<<<<boatDataList>>>>   "+  boatDataList.length);
...
$('#gpslattitudeDec').keyup(function(){
 ...
 $('#applongitudeDec').keyup(function(){
         ...
         $('#satLattitude').keyup(function(){
        	var dd=$('#satLattitude').val();
        	var deg = dd | 0; // truncate dd to get degrees
            var frac = Math.abs(dd - deg); // get fractional part
            var min = (frac * 60) | 0; // multiply fraction by 60 and truncate
            var sec = frac * 3600 - min * 60;
            ...
            if(dd==''){
            	 ...
            	 $('#satLattitudeDeg').val(show);
            }
           
        })
        
 $('#satLongitude').keyup(function(){
        	var dd=$('#satLongitude').val();
        	var deg = dd | 0; // truncate dd to get degrees
            var frac = Math.abs(dd - deg); // get fractional part
            var min = (frac * 60) | 0; // multiply fraction by 60 and truncate
            var sec = frac * 3600 - min * 60;
